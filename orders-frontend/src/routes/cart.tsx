...
function CartPage() {
    ...
    async function handleRegister(request: {username: string; password: string; displayName: string}) {
    ...
    async function checkout() {
        if (!user) {
            setAuthMode("login");
            toast.info("Sign in to checkout", {description: "Your cart will stay here after authentication."});
            return;
        }
        ...
        if (requestRide && (!rideForm.pickupLocation || !rideForm.dropoffLocation || Number(rideForm.distanceKm) <= 0)) {
            toast.error("Pickup, dropoff, and distance are required for a delivery ride");
            return;
        }
        ...
        try {
            ...
            toast.success("Order placed", {
                description: requestRide
                    ? `Order ${order.id.slice(0, 8)} is pending and a ride was requested.`
                    : `Order ${order.id.slice(0, 8)} is pending.`,
            });
        } catch (error) {
            toast.error("Checkout failed", {description: error instanceof Error ? error.message : "Please review your cart and try again."});
        }
    }
                                    ...
                                    <CardContent className="p-4">
                                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                                            <div>
                                                ...
                                                <p className="mt-1 text-sm text-muted-foreground">{item.product.sku}</p>
                            ...
                            <CardContent className="space-y-4">
                                        ...
                                        <div className="grid gap-3">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <Input
                                                    ...
                                                    onChange={(event) => setRideForm({...rideForm, distanceKm: event.target.value})}
                                                    required={requestRide}
                                                />
                                            </div>
                                        </div>
                                ...
                                {!user ? (
                                    <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <Button type="button" variant="outline" onClick={() => setAuthMode("register")}>
                                            Create account
                                        </Button>
                                    </div>
                                ) : null}
                                {createOrder.error ? (
                                    <ErrorState message={createOrder.error instanceof Error ? createOrder.error.message : "Checkout failed"}/>
                                ) : null}
                            </CardContent>
                        </Card>
                    </div>
