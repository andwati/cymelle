import {createUser, disableUser, getCustomers, getDrivers, getUsers, updateUser} from "#/api/users";
...
export function useUpdateUser() {
    ...
    return useMutation({
        mutationFn: ({id, request}: {id: string; request: AdminUserRequest}) => updateUser(id, request),
        onSuccess: () => queryClient.invalidateQueries({queryKey: userQueryKeys.all}),
    });
}
