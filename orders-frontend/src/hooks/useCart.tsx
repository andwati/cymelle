...
export function CartProvider({children}: {children: React.ReactNode}) {
    ...
    function addItem(product: Product, quantity: number) {
        const safeQuantity = Math.min(Math.max(quantity, 1), product.availableQuantity);
                ...
                return currentItems.map((item) =>
                    item.product.id === product.id
                        ? {
                            ...item,
                            product,
                            quantity: Math.min(item.quantity + safeQuantity, product.availableQuantity),
                        }
                        : item,
                );
            }
    ...
    function updateQuantity(productId: string, quantity: number) {
        setItems((currentItems) =>
            currentItems.flatMap((item) => {
                ...
                const nextQuantity = Math.min(Math.max(quantity, 0), item.product.availableQuantity);
